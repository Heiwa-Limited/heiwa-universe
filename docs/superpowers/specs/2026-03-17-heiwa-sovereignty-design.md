...
- **Implementation**:
  ...
  - Update all CRUD methods (e.g., `record_run`, `create_mission`) to _only_ execute if `self.stdb` is truthy. If STDB is missing, log a critical warning and return early (stateless mode).
