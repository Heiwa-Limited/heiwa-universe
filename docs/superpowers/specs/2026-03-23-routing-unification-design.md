...
2. **LocalLLMEngine** (`packages/heiwa_cognition/heiwa_cognition/llm.py`) — API inference layer with its own hardcoded tier chain (`_tier_chain()`). Routes: Gemini Flash -> Gemini Pro -> CLI tools -> Ollama. Used by enrichment, intent classification, planner, audit, HeiwaClaw reflex adapter, chat. Ignores ComputeRouter entirely.
...
- New facade functions `llm_generate()` and `llm_generate_with_plan()` in `heiwa_cognition`
- All 5+ callers migrate from `engine.generate(prompt, complexity)` to `llm_generate(prompt, intent, risk)`
...
- `transport` values are exactly the set from `ProviderRegistry`: `api_http`, `cli_stdio`, `local_http`. `gateway_websocket` is out of scope for LLM inference — that transport is for Class 3 agentic tasks routed through HeiwaClaw/ToolMesh.
...
The facade is the only new default path. `llm_generate_with_plan()` exists for inspection, tests, and debugging — not as a regular call path.

### Fallback Assembly

`ComputeRouter.route_inference()` assembles the `RoutedPlan` from:

1. STDB `model_tiers` table — filtered by intent strengths, capability class, privacy, runtime compatibility, enabled status
...
Fallbacks are selected from the same filtered tier set, ordered by cost (cheapest first), and must preserve the capability class of the primary. No hardcoded fallback chains.

## Execution Flow

1. Caller invokes `llm_generate(prompt, intent="enrichment", risk="low")`
2. Facade calls `ComputeRouter.route_inference("enrichment", "low")` -> `RoutedPlan`
...
4. On failure: facade walks `plan.fallbacks` in order, records failure in rate ledger
5. On chain exhaustion (all targets failed):
   - If `retry_policy == "exhaust_then_reroute"`: call `ComputeRouter.route_inference()` again with updated ledger state, try the new plan once
   - If `retry_policy == "reroute_immediately"`: this already happened at step 4
...
Each migration is a single-file change. Tests run after each.

1. `packages/heiwa_cognition/heiwa_cognition/enrichment.py` -> `llm_generate(prompt, intent="enrichment", risk="low")`
2. `packages/heiwa_cognition/heiwa_cognition/intent.py` -> `llm_generate(prompt, intent="classification", risk="low")`
3. `packages/heiwa_cognition/heiwa_cognition/planner.py` -> `llm_generate(prompt, intent="planning", risk="medium")`
4. `packages/heiwa_sdk/heiwa_sdk/audit.py` -> `llm_generate(prompt, intent="audit", risk="low")`
5. `packages/heiwa_sdk/heiwa_sdk/heiwaclaw/adapters/reflex.py` -> `llm_generate(prompt, intent=..., risk=...)`
6. `apps/heiwa_hub/agents/heiwaclaw.py` -> `llm_generate(prompt, intent=..., risk=...)`
7. `apps/heiwa_hub/chat.py` — last, since it's already coupled and pending removal
...
- Update `apps/heiwa_hub/cognition/llm_local.py` re-export shim to export facade instead
