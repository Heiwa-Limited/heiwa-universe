...
|---|---|---|---|---|---|---|
| L0.1 | ConfigRoot: single resolver owns per-user state root, platform-correct, first-run creation, sole authority | Extend `HeiwaPaths` (rename-in-place API): add `ensure()` first-run creation; document sole-authority rule; export from `heiwa_config` | `crates/heiwa_config/src/lib.rs` | — | `cargo test -p heiwa-config`; L0 gate greps | todo |
| L0.2 | Single-seat audit: no hardcoded operator identity, machine name, or home path outside resolver | Reroute independent `~/.heiwa`/HOME resolutions through `heiwa_config`; remove hardcoded identity (`devon-canonical` in `heiwa_provider::login_heiwa` confirmed; audit sweep pending) | `crates/heiwa_provider/src/lib.rs` (+ audit findings) | L0.1, audit report | L0 gate: home-path grep gate with allowlist | doing |
