...
| D-4 | The desktop app **did not build in release at all**. Host linker (`ld-27037`) emits proc-macro dylibs with a mis-aligned LINKEDIT string pool, which rustc then cannot `dlopen`. Reproduced again after a full `cargo clean --release` on 2026-08-20, proving the profile workaround was insufficient | fixed: the shared Tauri build wrapper selects Rust's bundled Mach-O linker on Apple Silicon macOS, preserves explicit operator overrides, and is used by local packaging plus CI; local staging uses an isolated Cargo target because case-insensitive macOS treats Tauri's `Heiwa` and CLI `heiwa` outputs as the same path; `[profile.release.build-override]` remains only a compile-time cost control |
