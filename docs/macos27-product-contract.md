...
| Resource onboarding | Implemented passive catalog and reopenable panel; provider API-key connections and selected Calendar import use current connector surfaces | App/CLI/account distinctions, error/retry, no fabricated resources; broader connector coverage and richer source detail remain |
...
| Provider setup/routing | In-app API-key connection, verification, disconnect, OpenRouter key admission, and registered-account projection are implemented; exact route selection still belongs to Rust | Exact channel/model/effort/modality admission, account-specific image execution, budgets, measured feedback |
| Apple productivity | Selected Calendar import now uses a bounded Swift EventKit reader with stable source IDs and atomic reconciliation; Mail remains metadata-only and broader actions are unfinished | Fresh-user permission prompts, round-trip update/completion/send proof, revoked access, and broader connector coverage |
| Public installation | The macOS 27 bundle now embeds the exact CLI and Apple helper, provisions `~/.heiwa` on first launch, and has a fail-closed signing/notarization workflow | Developer ID credentials, clean-machine Gatekeeper/notarization proof, and the first signed public release |
