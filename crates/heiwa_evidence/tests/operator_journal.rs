...
fn read_after_skips_a_truncated_tail_line() {
    ...
    let journal = OperatorJournal::new(dir.path().to_path_buf()).unwrap();
    journal
        .append(&event("e1", "thread-a", OperatorEventType::UserMessage, json!({"text":"hi"})))
        .unwrap();
    journal
        .append(&event("e2", "thread-a", OperatorEventType::AssistantCompleted, json!({"text":"hello"})))
        .unwrap();
    ...
    use std::io::Write;
    write!(file, "{{\"v\":1,\"at_ms\":1,\"kind\":\"operator_events\",\"record\":{{\"event_id\":\"e3\"").unwrap();
    drop(file);

    let page = journal.read_after(None, 50).unwrap();
    assert_eq!(
        page.events.iter().map(|row| row.event.event_id.as_str()).collect::<Vec<_>>(),
        vec!["e1", "e2"],
        "valid events survive the truncated tail"
    );
    assert_eq!(page.skipped_lines, 1, "the truncated tail line is counted, not fatal");
}
...
fn next_cursor_echoes_input_when_empty_and_advances_when_events_return() {
    ...
    assert_eq!(empty_page.next_cursor.as_deref(), Some(first.cursor.as_str()));
...
fn concurrent_operator_journals_do_not_tear_lines() {
    ...
    let unique_ids: std::collections::HashSet<_> =
        page.events.iter().map(|row| row.event.event_id.clone()).collect();
