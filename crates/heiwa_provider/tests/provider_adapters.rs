...
impl ProviderAdapter for MockAdapter {
    async fn send(
        ...
        stream_tx.send(StreamEvent::Token("hello from mock".to_string())).await.ok();
        stream_tx.send(StreamEvent::Done(Default::default())).await.ok();
        Ok(())
    }
...
async fn test_mock_adapter_streaming() {
    ...
    adapter.send("mock-model", &messages, tx).await.expect("send failed");
    ...
    while let Some(event) = rx.recv().await {
        match event {
            ...
            StreamEvent::Done(_) => { got_done = true; break; }
