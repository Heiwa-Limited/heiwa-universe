...
CI economy: required PR CI is a hard sub-minute production gate; the full Rust test and static lanes run in parallel on the organization's existing Blacksmith free-tier runners. Every protected-main commit reruns fast CI plus exhaustive cross-platform certification, and releases require both workflows at the exact commit. Deploy remains dispatch-only. Local verification is the default; remote CI is the promotion gate, not a development loop.
