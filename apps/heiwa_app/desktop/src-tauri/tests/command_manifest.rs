...
/// Commands declared in the app manifest (`COMMANDS` in build.rs) that are
/// deliberately not granted in `capabilities/default.json` today. Every
/// entry needs a reason: an ungranted command is simply unreachable from
/// the webview (Tauri denies any command with no matching `allow-*`
/// permission in an applicable capability), which is the correct
/// least-privilege default until a real caller needs it. Added in review
/// round 1 (Opus, on PR #131), which pointed out that granting a command
/// with zero frontend callers is not actually least privilege.
const INTENTIONALLY_UNGRANTED: &[&str] = &[
    // No frontend call site invokes this today (grepped every `invoke(...)`
    // across apps/heiwa_app/desktop/src/**/*.{ts,tsx} during L-009's
    // review). Its own doc comment in lib.rs describes an intended "the
    // shell reads this to explain why nothing works" consumer that was
    // never wired up. Kept registered (not removed from generate_handler!)
    // because removing a command outright is a bigger, riskier change than
    // this task's scope calls for; ungranted is safe by construction, and
    // granting it later is a one-line capabilities/default.json change
    // whenever a real caller exists.
    "runtime_startup",
];

fn granted_permissions() -> Vec<String> {
    let json = fs::read_to_string(manifest_dir().join("capabilities/default.json"))
    ...
    value["permissions"]
        .as_array()
        .expect("capabilities/default.json's \"permissions\" must be an array")
        .iter()
        .map(|entry| {
            entry
                .as_str()
                .expect("each permission entry must be a string")
                .to_string()
        })
        .collect()
}

/// Mirrors tauri-utils's own autogenerated-permission naming
/// (`tauri-utils-2.9.3/src/acl/build.rs:290`): underscores become dashes in
/// the permission identifier, even though the command name itself keeps
/// them.
fn allow_permission_for(command: &str) -> String {
    format!("allow-{}", command.replace('_', "-"))
}

#[test]
fn every_manifest_command_is_either_granted_or_explicitly_ungranted() {
    let granted = granted_permissions();
    let ungranted_without_reason: Vec<_> = app_manifest_commands()
        .into_iter()
        .filter(|command| {
            !granted.contains(&allow_permission_for(command))
                && !INTENTIONALLY_UNGRANTED.contains(&command.as_str())
        })
        .collect();
    assert!(
        ungranted_without_reason.is_empty(),
        "these app-manifest commands are neither granted in capabilities/default.json nor \
         listed in INTENTIONALLY_UNGRANTED with a reason: {ungranted_without_reason:?}"
    );
}

#[test]
fn intentionally_ungranted_commands_stay_actually_ungranted() {
    // Guards INTENTIONALLY_UNGRANTED itself from going stale: if someone
    // grants runtime_startup later without deleting its entry here, that is
    // a silent contradiction between this list and the capability file.
    let granted = granted_permissions();
    let wrongly_granted: Vec<_> = INTENTIONALLY_UNGRANTED
        .iter()
        .filter(|command| granted.contains(&allow_permission_for(command)))
        .collect();
    assert!(
        wrongly_granted.is_empty(),
        "these commands are listed as INTENTIONALLY_UNGRANTED but ARE granted in \
         capabilities/default.json -- remove them from INTENTIONALLY_UNGRANTED: \
         {wrongly_granted:?}"
    );
}

#[test]
fn intentionally_ungranted_commands_are_still_in_the_app_manifest() {
    // If a command is ever removed from generate_handler!/build.rs
    // entirely, its INTENTIONALLY_UNGRANTED entry becomes dead
    // documentation for a command that no longer exists.
    let manifest = app_manifest_commands();
    let stale: Vec<_> = INTENTIONALLY_UNGRANTED
        .iter()
        .filter(|command| !manifest.contains(&command.to_string()))
        .collect();
    assert!(
        stale.is_empty(),
        "INTENTIONALLY_UNGRANTED lists commands not in the app manifest -- remove the stale \
         entry: {stale:?}"
    );
}
