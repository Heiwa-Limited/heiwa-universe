...
@command("/inspect", "Legacy backend inspector (retired)")
async def cmd_inspect(ctx: CLIContext, args: str = "") -> None:
    del ctx, args
    console.print(
        "  [yellow]Retired:[/yellow] Python backend inspection was removed. "
        "Use the installed Rust runtime and its evidence/read-model commands."
    )


@command("/start", "Legacy Python hub launcher (retired)")
async def cmd_start(ctx: CLIContext, args: str = "") -> None:
    del ctx, args
    console.print(
        "  [yellow]Retired:[/yellow] the Python hub launcher was removed. "
        "Run [bold]heiwa app start[/bold] from the installed Rust runtime."
    )
