...
class HubStateService:
    ...
    def get_provider_accounts(self, provider_id: str | None = None, node_id: str | None = None) -> list[dict[str, Any]]:
        return self.db.list_provider_accounts(provider_id=provider_id, node_id=node_id, limit=100)

    def get_provider_status(self, provider_id: str) -> dict[str, Any] | None:
        rows = self.db.list_provider_accounts(provider_id=provider_id, limit=10)
        return rows[0] if rows else None

    def get_missions(self, status: str | None = None, limit: int = 50) -> list[dict[str, Any]]:
        return self.db.get_missions(status=status, limit=max(1, int(limit)))
    ...
    def get_mission_detail(self, mission_id: str) -> dict[str, Any] | None:
        ...
        mission["cell_runs"] = self.db.get_cell_runs(mission_id=mission_id, limit=100)
        mission["artifacts"] = self.db.list_artifacts(mission_id=mission_id, limit=100)
        return mission
    ...
    def get_history(self, limit: int = 20) -> dict[str, Any]:
        return {
            "session_summaries": self.db.list_session_summaries(limit=max(1, int(limit))),
            "completed_missions": self.db.get_missions(status="completed", limit=max(1, int(limit))),
            "failed_missions": self.db.get_missions(status="failed", limit=max(1, int(limit))),
            "artifacts": self.db.list_artifacts(limit=max(1, int(limit))),
            "recent_runs": self.db.get_runs(limit=max(1, int(limit))),
        }
