...
async def lifespan(app: FastAPI):
    # Startup: Bootstrap the DB
    try:
        db.init_db()
    except Exception as e:
        print(f"[CRITICAL] DB Init failed: {e}")
        # In standard FastAPI/Starlette, raising here might crash startup, which is what we want.
...
def auth_callback(code: Optional[str] = None):
    ...
    For now, we just acknowledge the handshake. 
    In the future, we can exchange 'code' for a user token to log them into a dashboard.
    """
    return {"status": "authorized", "message": "You can close this window.", "code_received": bool(code)}
