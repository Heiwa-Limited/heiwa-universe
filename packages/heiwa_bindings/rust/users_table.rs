...
impl UsersTableAccess for super::RemoteTables {
    fn users(&self) -> UsersTableHandle<'_> {
        UsersTableHandle {
            imp: self.imp.get_table::<User>("users"),
...
/// Access to the `user_id` unique index on the table `users`,
/// which allows point queries on the field of the same name
/// via the [`UsersUserIdUnique::find`] method.
///
/// Users are encouraged not to explicitly reference this type,
/// but to directly chain method calls,
/// like `ctx.db.users().user_id().find(...)`.
pub struct UsersUserIdUnique<'ctx> {
    imp: __sdk::UniqueConstraintHandle<User, String>,
    phantom: std::marker::PhantomData<&'ctx super::RemoteTables>,
}

impl<'ctx> UsersTableHandle<'ctx> {
    /// Get a handle on the `user_id` unique index on the table `users`.
    pub fn user_id(&self) -> UsersUserIdUnique<'ctx> {
        UsersUserIdUnique {
...
impl<'ctx> UsersUserIdUnique<'ctx> {
...
pub(super) fn register_table(client_cache: &mut __sdk::ClientCache<super::RemoteModule>) {
    let _table = client_cache.get_or_make_table::<User>("users");
...
pub trait usersQueryTableAccess {
    ...
    fn users(&self) -> __sdk::__query_builder::Table<User>;
}
