// THIS FILE IS AUTOMATICALLY GENERATED BY SPACETIMEDB. EDITS TO THIS FILE
// WILL NOT BE SAVED. MODIFY TABLES IN YOUR MODULE SOURCE CODE INSTEAD.

#![allow(unused, clippy::all)]
use spacetimedb_sdk::__codegen::{self as __sdk, __lib, __sats, __ws};

#[derive(__lib::ser::Serialize, __lib::de::Deserialize, Clone, PartialEq, Debug)]
#[sats(crate = __lib)]
pub struct Session {
    pub id: String,
    pub platform: String,
    pub agent_id: String,
    pub created_at: u64,
    pub expires_at: u64,
}

impl __sdk::InModule for Session {
    type Module = super::RemoteModule;
}

/// Column accessor struct for the table `Session`.
///
/// Provides typed access to columns for query building.
pub struct SessionCols {
    pub id: __sdk::__query_builder::Col<Session, String>,
    pub platform: __sdk::__query_builder::Col<Session, String>,
    pub agent_id: __sdk::__query_builder::Col<Session, String>,
    pub created_at: __sdk::__query_builder::Col<Session, u64>,
    pub expires_at: __sdk::__query_builder::Col<Session, u64>,
}

impl __sdk::__query_builder::HasCols for Session {
    type Cols = SessionCols;
    fn cols(table_name: &'static str) -> Self::Cols {
        SessionCols {
            id: __sdk::__query_builder::Col::new(table_name, "id"),
            platform: __sdk::__query_builder::Col::new(table_name, "platform"),
            agent_id: __sdk::__query_builder::Col::new(table_name, "agent_id"),
            created_at: __sdk::__query_builder::Col::new(table_name, "created_at"),
            expires_at: __sdk::__query_builder::Col::new(table_name, "expires_at"),
        }
    }
}

/// Indexed column accessor struct for the table `Session`.
///
/// Provides typed access to indexed columns for query building.
pub struct SessionIxCols {
    pub id: __sdk::__query_builder::IxCol<Session, String>,
}

impl __sdk::__query_builder::HasIxCols for Session {
    type IxCols = SessionIxCols;
    fn ix_cols(table_name: &'static str) -> Self::IxCols {
        SessionIxCols {
            id: __sdk::__query_builder::IxCol::new(table_name, "id"),
        }
    }
}

impl __sdk::__query_builder::CanBeLookupTable for Session {}
